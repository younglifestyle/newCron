...
func NewWorker(conf *Config) (w *Worker) {
	w = &Worker{
		Config:         conf,
		ID:             conf.HostName,
		Client:         etcd.NewClient(conf.etcdConf),
	...
	w.logger.Info("agent info :", zap.String("name", conf.AppIP+":"+conf.HostName))
...
func (w *Worker) delJob(id string) {
	...
	w.logger.Error("Worker.delJob:delete a job", zap.String("jobId", id))
...
func (w *Worker) addJob(job *Job) {
	...
	if util.InStringArray(job.Nodes, w.HostName) < 0 {
		// ignore
		w.logger.Info("Worker.addJob: Nodes do not contain current node, skip it.", zap.String("jobId", job.ID))
		return
	}
	...
	if job.JobType == TypeAlone {
		err := job.Lock()
		if err != nil {
			w.logger.Info("failed to lock job. ignore it", zap.String("jobId", job.ID))
			return
		}
	}

	w.logger.Info("Worker.addJob: add a job", zap.String("jobId", job.ID), zap.Any("job", job))
...
func (w *Worker) delCmd(cmd *Cmd) {
	...
	w.logger.Infof("job[%s] rule[%s] timer[%s] has deleted", cmd.Job.ID, cmd.JobRule.ID, cmd.JobRule.CronExp)
}
...
func (w *Worker) modCmd(cmd *Cmd) {
	...
	sch := c.JobRule.CronExp
	*c = *cmd
	c.schEntryID = entryID

	// 节点执行时间改变，更新 cron
	// 否则不用更新 cron
	if c.JobRule.CronExp != sch {
		w.Cron.Remove(entryID)
		c.schEntryID = w.Cron.Schedule(c.JobRule.Schedule, c)
	}

	w.logger.Infof("job[%s]rule[%s] timer[%s] has updated", c.Job.ID, c.JobRule.ID, c.JobRule.CronExp)
}
...
func (w *Worker) addCmd(cmd *Cmd) {
	...
	w.logger.Infof("job[%s] rule[%s] timer[%s] has added",
		cmd.Job.ID, cmd.JobRule.ID, cmd.JobRule.CronExp)
	return
}
...
func (w *Worker) GetJobContentFromKv(key []byte, value []byte) (*Job, error) {
	...
	job.mutex, err = etcd.NewMutex(w.Client, LockKeyPrefix+job.ID, concurrency.WithTTL(30))
	if err != nil {
		return nil, err
	}
...
func (w *Worker) handleOnceEv(ev clientv3.WatchResponse) {
	for _, event := range ev.Events {
		...
		case event.IsCreate(), event.IsModify():
			...
			if err != nil {
				w.logger.Error("get job from kv failed", zap.String("err", err.Error()))
				continue
			}
