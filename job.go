...
func (j *Job) Run(taskOptions ...TaskOption) error {
	...
	if err != nil {
		j.logger.Error("read script file failed", zap.String("err", err.Error()))
		...
		j.logger.Error("script path is a dir", zap.String("script", j.Script))
...
func (j *Job) Unlock() {
	err := j.mutex.Unlock()
	if err != nil {
		j.logger.Error("unlock failed", zap.Error(err))
	}
}

type JobRule struct {
	ID      string `json:"id"`
	CronExp string `json:"timer"` // cron表达式 expression
...
func (rule *JobRule) Valid() error {
	...
	if len(rule.CronExp) == 0 {
		return errors.New("invalid job rule, empty timer.")
	}

	sch, err := myParser.Parse(rule.CronExp)
	if err != nil {
		return fmt.Errorf("invalid JobRule[%s], parse err: %s", rule.CronExp, err.Error())
	}
...
func (c *Cmd) Run() error {
	if c.Job.RetryCount <= 0 {
		err := c.Job.Run()
		if err != nil {
			c.logger.Info("job run failed : ", zap.Error(err))
		}
		return err
	}
